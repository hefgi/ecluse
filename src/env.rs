...
mod tests {
    ...
    fn basic_env_vars_always_present() {
        let env = build_env(2, 200, "host", None, None, &[]);
    ...
    fn app_port_sets_port_and_ecluse_app_port() {
        let env = build_env(1, 100, "host", Some(3100), None, &[]);
    ...
    fn no_port_when_app_port_none() {
        let env = build_env(1, 100, "container", None, None, &[]);
        assert!(!env.contains_key("PORT"));
        assert!(!env.contains_key("ECLUSE_APP_PORT"));
    }

    #[test]
    fn database_name_sets_database_url_and_ecluse_database() {
        let env = build_env(1, 100, "host", None, Some("myapp_feat"), &[]);
        assert_eq!(env["DATABASE_URL"], "postgres://localhost/myapp_feat");
        assert_eq!(env["ECLUSE_DATABASE"], "myapp_feat");
    }

    #[test]
    fn data_service_postgres_sets_database_url() {
        let env = build_env(1, 100, "hybrid", None, None, &[("postgres".into(), 5532)]);
    ...
    fn data_service_redis_sets_redis_url() {
        let env = build_env(1, 100, "hybrid", None, None, &[("redis".into(), 6479)]);
    ...
    fn data_service_unknown_sets_port_only() {
        let env = build_env(1, 100, "hybrid", None, None, &[("mailhog".into(), 8125)]);
    ...
    fn write_env_file_creates_sorted_file() {
        ...
        let mut env = HashMap::new();
        env.insert("ECLUSE_SLOT".into(), "1".into());
        env.insert("ECLUSE_MODE".into(), "host".into());
        env.insert("PORT".into(), "3100".into());
